...
namespace TheReader.Services.BookServiceTests
{
	...
	public class BookAndGenreServiceTests
	{
		...
		public void Setup()
		{
			bookId = 2;
			genreId = 2;
			userId = "641ca250 - 7c7a - 40a5 - 8e3c - 657714fb3d4a";
			//Genres
			Fantasy = new Genre()
			{
				Id = 1,
		...
		public async Task BookExistsAsync_ShouldReturnFalse()
		{
			...
			bool result = await bookService.BookExistsAsync(notBookExist);
		...
		public async Task LastFourBooksAsync_ShouldReturnCorrectCount()
		{
			...
			var result = await bookService.LastFourBooksAsync();
		...
		public async Task DeleteBookAsync_ShouldReturnCorrectCount()
		{
			...
			var expected = new DeleteBookViewModel()
			{
				Id = book.Id,
				Author = book.Author,
				Title = book.Title,
			...
			var result = bookService.DeleteBookAsync(bookId);
		...
		public async Task GetBookByIdAsync_ShouldReturnCorrectItem()
		{
			...
			Assert.Multiple(() =>
			{
				...
				Assert.That(expected.ISBN, Is.EqualTo(result.ISBN));
				Assert.That(expected.GenreId, Is.EqualTo(result.GenreId));
				Assert.That(expected.Author, Is.EqualTo(result.Author));
				Assert.That(expected.Language, Is.EqualTo(result.Language));
				Assert.That(expected.Pages, Is.EqualTo(result.Pages));
				Assert.That(expected.Weight, Is.EqualTo(result.Weight));
			});

		}

		[Test]
		public async Task EditBookAsync_ShouldReturnModifiedItem()
		{
			var model = new BookFormViewModel()
			{
				Title = "Edited Test",
			...
			Assert.Multiple(() =>
			{
				...
				Assert.That(result.ISBN, Is.EqualTo(model.ISBN));
				Assert.That(result.GenreId, Is.EqualTo(model.GenreId));
				Assert.That(result.Author, Is.EqualTo(model.Author));
				Assert.That(result.Language, Is.EqualTo(model.Language));
				Assert.That(result.Pages, Is.EqualTo(model.Pages));
				Assert.That(result.Weight, Is.EqualTo(model.Weight));
			});
		}
		...
		public async Task GetDetailsByIdAsync_ShouldReturnRightItem()
		{
			...
			var result = await bookService.GetDetailsByIdAsync(bookId);
		...
		public async Task DeleteBookConfirmAsync_ShouldTurnIsDeleteToTrue()
		{
			...
			await bookService.DeleteBookConfirmAsync(bookId);

			Assert.That(item.IsDeleted, Is.True);
		}

		[Test]
		public async Task AllIBooksByChoosenGenreAsync_ShouldReturnArrayOfItems()
		{
			var genreName = "Биография";
			var expected = await dbContext
				.Books
				.Where(i => i.Genre.Name == genreName)
				.ToListAsync();

			var result = await bookService.AllIBooksByChoosenGenreAsync(genreName);
